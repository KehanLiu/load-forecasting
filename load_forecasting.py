...
df_raw = pd.read_csv('E:\Git\GITHUB\Electric-Power-Hourly-Load-Forecasting-using-Recurrent-Neural-Networks-master\hourly_load_2016.csv', header=None)
# numpy array
df_raw_array = df_raw.values
# daily load, 213 days total
...
# build the model
model = Sequential()
# layer 1: LSTM 如果return_sequences=True：返回形如（samples，timesteps，output_dim）的3D张量，否则，返回形如（samples，output_dim）的2D张量， 这里没填num_units的数量，该值表示隐藏层神经元的个数
...
test_result_plus_shifted_value = np.hstack((predicted_values, y_test)) + shifted_value
np.savetxt('output_load_forecasting_result_plus_shifted_value.csv', test_result_plus_shifted_value, fmt='%.2f',delimiter=',',header="#real-data,#pred-data")
